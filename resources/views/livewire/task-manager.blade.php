<div
    class="w-xl mx-auto bg-gray-700 border border-gray-500 shadow-lg rounded-lg overflow-hidden mt-16 text-gray-200">
    <div class="px-4 py-2">
        <h1 class="font-bold text-lg">What will you work on today?</h1>
    </div>
    <form class="w-full mx-auto px-4 py-2" wire:submit.prevent='addUpdateTask'>
                    ...
                    <li class="{{ $task->is_done ? 'bg-gray-800' : 'bg-gray-900' }} w-full block"
                        ...
                        <div class="flex items-center p-4 gap-4">
                            ...
                            <p class="ml-3 text-gray-200 flex justify-between w-full">
                                <span
                                    class="text-lg font-medium {{ $task->is_done ? 'line-through text-gray-600' : '' }}">{{ $task->body }}</span>
                                <span class="text-md font-light text-gray-500">{{ $task->project }}</span>
                            </p>
                            {{-- /Task Body & Project --}}
                            {{-- Task Created At Human Form --}}
                            <span class="text-xs font-light text-center">created
                                {{ $task->created_at->diffForHumans() }}</span>
                            {{-- /Task Created At Human Form --}}
                            {{-- Task Updated At Human Form --}}
                            @if ($task->is_done)
                                <span class="text-xs font-light text-center">done
                                    {{ $task->done_at->diffForHumans() }}</span>
                            @endif
                            {{-- /Task Updated At Human Form --}}
                            {{-- Task Status --}}
                            <button
