<div>
    ...
    <section class="mt-4 py-4 rounded-lg">
        ...
        <ul class="flex flex-col gap-1" wire:sortable="updateTaskListPriority">
            ...
            <li class="{{ $task->is_done ? 'bg-gray-800' : 'bg-gray-900' }} w-full block" wire:sortable.item="{{ $task->id }}" wire:key="task-{{ $task->id }}">
                <div class="flex items-center p-4 gap-4">
                    ...
                    @if($task->is_done)
                    <span class="text-xs font-light text-center">done {{ $task->done_at->diffForHumans() }}</span>
                    @endif
                    {{-- /Task Updated At Human Form --}}
                    {{-- Task Status --}}
                    <button
